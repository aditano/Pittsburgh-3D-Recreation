...
import { ROOT, unproject } from './osm.mjs';

const d = JSON.parse(readFileSync(join(ROOT, 'public/data/pittsburgh.json'), 'utf8'));
...
for (const [name, x, z] of spots) {
  ...
  if (usgs === null) {
    failedQueries++;
    console.log(name.padEnd(30), ours.toFixed(1).padStart(11), '   (query failed)');
    continue;
  }
...
if (failedQueries) console.log(`${failedQueries} USGS queries failed (informational; the comparison that did return is above)`);
